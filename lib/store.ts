...
import type { CodePost, CommentItem, CompletedWork, NewPostInput, SortMode, SupabasePostRow } from "./types";
...
function readLocal(): CodePost[] {
  ...
  const posts = (JSON.parse(raw) as CodePost[]).map(normalizePost);
  const postIds = new Set(posts.map((post) => post.id));
  const missingSeeds = seedPosts.filter((post) => !postIds.has(post.id));
  if (!missingSeeds.length) return posts;
  const merged = [...missingSeeds, ...posts];
...
export async function listPosts(sort: SortMode, query = "") {
  if (supabase) {
    const { data } = await supabase
      .from("posts")
      .select("id,kind,title,code,description,problem,target_user,budget,reward,deadline,theme_status,completed_works,screenshot_url,screenshot_urls,archive_url,archive_name,archive_size,language,ai_tool,x_account,created_at,likes_count,comments:comments(id,body,created_at)")
    ...
    if (data) {
      return sortPosts(
        (data as SupabasePostRow[]).map((item) => normalizePost({
          ...
          themeStatus: item.theme_status ?? "open",
          completedWorks: item.completed_works ?? [],
...
export async function createPost(input: NewPostInput) {
  ...
  if (supabase) {
    const { data } = await supabase
      .rpc("create_post_with_password", {
        ...
        post_problem: input.problem || null,
        post_target_user: input.targetUser || null,
        post_budget: input.budget || null,
        post_reward: input.reward || null,
        post_deadline: input.deadline || null,
  ...
  const post: CodePost = {
    ...
    themeStatus: kind === "theme" ? "open" : undefined,
    completedWorks: kind === "theme" ? [] : undefined,
...
export async function startTheme(postId: string) {
  if (supabase) {
    await supabase.rpc("start_theme", { post_id: postId });
  }
  const posts = readLocal().map((post) =>
    post.id === postId ? { ...post, themeStatus: "building" as const } : post
  );
  writeLocal(posts);
}

export async function addCompletedWork(postId: string, title: string, url?: string) {
  ...
  if (supabase) {
    await supabase.rpc("add_completed_work", { post_id: postId, work });
  }

  const posts = readLocal().map((post) =>
    post.id === postId
      ? { ...post, themeStatus: "completed" as const, completedWorks: [...(post.completedWorks ?? []), work] }
      : post
  );
  writeLocal(posts);
}
...
function normalizePost(post: CodePost): CodePost {
  ...
  return {
    ...
    themeStatus: post.themeStatus ?? (post.kind === "theme" ? "open" : undefined),
    completedWorks: post.completedWorks ?? []
  };
}
