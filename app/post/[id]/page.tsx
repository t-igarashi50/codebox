...
import { addComment, addCompletedWork, deletePost, formatBytes, formatTime, getPost, getPostImages, hasSharedCode, likePost, startTheme } from "../../../lib/store";
...
export default function PostPage() {
  ...
  async function handleStartTheme() {
    await startTheme(currentPost.id);
    await refresh();
  }

  async function submitCompletedWork(event: React.FormEvent) {
    event.preventDefault();
    if (!completedTitle.trim()) return;
    await addCompletedWork(currentPost.id, completedTitle.trim(), completedUrl.trim());
    setCompletedTitle("");
    setCompletedUrl("");
    await refresh();
  }
  ...
  return (
    <main className="min-h-screen px-4 py-6">
      <article className="mx-auto max-w-5xl rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
        ...
        {currentPost.kind === "theme" ? (
          <ThemeDetail post={currentPost} onStart={handleStartTheme} />
        ) : (
          <>
      ...
      {currentPost.kind === "theme" ? (
        <section className="mx-auto mt-6 max-w-5xl rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
          <h2 className="font-black">完成報告</h2>
          <form onSubmit={submitCompletedWork} className="mt-4 grid gap-3 sm:grid-cols-[1fr_1fr_auto]">
            <input value={completedTitle} onChange={(event) => setCompletedTitle(event.target.value)} className="rounded-lg border border-slate-200 px-4 py-3 outline-none focus:border-blueprint" placeholder="完成した作品名" />
            <input value={completedUrl} onChange={(event) => setCompletedUrl(event.target.value)} className="rounded-lg border border-slate-200 px-4 py-3 outline-none focus:border-blueprint" placeholder="作品URL（任意）" />
            <button className="rounded-lg bg-blueprint px-5 py-3 font-bold text-white">報告</button>
          </form>
          <div className="mt-5 grid gap-3">
            {(currentPost.completedWorks ?? []).map((work) => (
              <a key={work.id} href={work.url || "#"} target={work.url ? "_blank" : undefined} rel="noreferrer" className="rounded-lg border border-blue-100 bg-blue-50 p-4 font-bold text-blueprint hover:bg-blue-100">
                {work.title}
              </a>
            ))}
            {!(currentPost.completedWorks ?? []).length ? <p className="text-sm text-slate-500">まだ完成報告はありません。</p> : null}
          </div>
        </section>
      ) : null}
...
function ThemeDetail({ post, onStart }: { post: CodePost; onStart: () => void }) {
    ...
    <div className="mt-5 space-y-5">
      <div className="rounded-lg border border-orange-100 bg-orange-50 p-4">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <p className="text-sm font-black text-bolt">テーマ募集</p>
            <p className="mt-1 text-sm font-bold text-slate-600">状態: {themeStatusLabel(post.themeStatus)}</p>
          </div>
          <button onClick={onStart} className="rounded-lg bg-bolt px-5 py-3 text-sm font-black text-white hover:bg-orange-600">
            このテーマを作る
          </button>
        </div>
      </div>

      {post.description ? <InfoBlock title="やりたいこと" body={post.description} /> : null}
      {post.problem ? <InfoBlock title="困っていること" body={post.problem} /> : null}
      {post.targetUser ? <InfoBlock title="想定ユーザー" body={post.targetUser} /> : null}

      <div className="grid gap-3 sm:grid-cols-3">
        <MiniInfo title="予算" value={post.budget} />
        <MiniInfo title="賞金" value={post.reward} />
        <MiniInfo title="募集期限" value={post.deadline} />
      </div>

      {images.length ? (
        <div className="grid grid-cols-2 overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
          {images.map((image, index) => (
            <img key={`${image}-${index}`} src={image} alt={`${post.title} 参考画像 ${index + 1}`} className={`h-full min-h-44 w-full object-cover ${images.length === 1 ? "col-span-2" : ""}`} />
          ))}
        </div>
      ) : null}
    </div>
...
function MiniInfo({ title, value }: { title: string; value?: string }) {
  return (
    <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
      ...
      <p className="mt-2 font-black text-ink">{value || "未設定"}</p>
    </div>
  );
}

function themeStatusLabel(status?: string) {
  if (status === "building") return "開発中";
  if (status === "completed") return "完成";
  if (status === "improving") return "改良中";
  return "募集中";
}
...
function ArchiveDownload({ post }: { post: CodePost }) {
  ...
  async function handleDownload() {
    await downloadFile(post.archiveUrl ?? "", post.archiveName || "codebox-work.zip");
  }
