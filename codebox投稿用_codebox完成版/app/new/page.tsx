...
export default function NewPostPage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    setMessage("");
    if (!title.trim() || !deletePassword.trim() || (!code.trim() && !description.trim() && !screenshotUrls.length && !archive)) {
  ...
  return (
    <main className="min-h-screen px-4 py-6">
      <form onSubmit={submit} className="mx-auto max-w-4xl rounded-lg border border-slate-200 bg-white p-5 shadow-soft">
        ...
        <input value={title} onChange={(event) => setTitle(event.target.value)} className="mt-2 w-full rounded-lg border border-slate-200 px-4 py-3 outline-none focus:border-bolt" placeholder="ASMR音声編集ツールを作った" />
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          <label className="block text-sm font-black">画像（最大4枚）</label>
        ...
        <div className="mt-5 flex items-center justify-between gap-3">
          <label className="block text-sm font-black">作品ZIP（任意）</label>
        ...
        <label className="mt-5 block text-sm font-black">コメント</label>
        <textarea value={description} onChange={(event) => setDescription(event.target.value)} className="mt-2 h-24 w-full rounded-lg border border-slate-200 px-4 py-3 outline-none focus:border-bolt" placeholder="例: 台本と音声を入れると、AIが編集候補を出してくれるやつ" />
        ...
        <label className="mt-5 block text-sm font-black">コード（任意）</label>
        <textarea value={code} onChange={(event) => setCode(event.target.value)} className="code-scroll mt-2 h-44 w-full rounded-lg border border-slate-200 bg-slate-950 px-4 py-3 font-mono text-sm leading-6 text-slate-100 outline-none focus:border-bolt sm:h-56" placeholder="見せたい人だけ貼る。メインの1ファイルだけでもOK。" />
